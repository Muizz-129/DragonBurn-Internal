...
private:
    ...
    void read_weapon(uintptr_t pawn, uintptr_t entity_list, char* out_name, size_t max_len, uint16_t& out_def_index) {
        ...
        // 1. Read the name string in a single `read_raw_mem` call (to avoid repeated loops and exception crashes).
        char dname[64] = { 0 };
        ...
        if (dname[0] != '\0') {
            ...
            // 2. Fast ID mapping to strstr (Zero memory allocation / speed)
        ...
        // 3. Fallback: Continue reading the Item Definition Index if the designer name fails to be detected
