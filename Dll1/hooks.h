...
namespace Hooks {
    ...
    inline std::atomic<bool> g_Unloading{ false }; // Safety flag during ejection
...
#define STEAM_FALLBACK_PRESENT_OFFSET 0x162200
    ...
    inline HRESULT NTAPI hkResizeBuffers(IDXGISwapChain* sc, UINT bc, UINT w, UINT h, DXGI_FORMAT fmt, UINT flags) {
        ...
        // Clear the old render target and empty the DirectX command queue
        Render::cleanup_render_target();

        // Call the original function to resize the CS2 buffer resolution
        HRESULT hr = oResizeBuffers(sc, bc, w, h, fmt, flags);

        // If the resolution is successfully changed and the swapchain remains valid, rebuild the RTV
        if (SUCCEEDED(hr) && Render::g_pDevice) {
            Render::g_pPinnedSwapChain = sc; // Update the active swapchain if the address changes
    ...
    inline HRESULT NTAPI hkPresent(IDXGISwapChain* sc, UINT sync, UINT flags) {
        ...
        // If the unload process is underway, continue calling the original without touching ImGui
    ...
    inline void unhook() {
        // 1. Lock render frame immediately
        g_Unloading.store(true, std::memory_order_seq_cst);

        // 2. Restore the original Steam Overlay cursor
        ...
        // 3. Allow a pause for any in-flight frames to complete
        Sleep(200);

        // DO NOT put "oPresent = nullptr;" here!
    }
}